...
use chrono::Utc;
use hearth_core::{
    Account, MissionCompletion, OwnedBlueprint, Platform, RecordId, WishIntent, WishlistEntry,
};
...
/// Mark a mission completed in this scope (idempotent). Returns the row with
/// any already-collected rewards loaded.
pub async fn add_mission_completion(
    pool: &DbPool,
    scope: Scope,
    mission_id: &str,
) -> Result<MissionCompletion> {
    if let Some(existing) = get_mission_completion(pool, scope, mission_id).await? {
        return Ok(existing);
    }
    let id = RecordId::new_v7();
    let completed_at = Utc::now();
    sqlx::query(
        "INSERT INTO mission_completions (id, mission_id, platform_id, account_id, completed_at) \
         VALUES (?, ?, ?, ?, ?)",
    )
    .bind(id.to_string())
    .bind(mission_id)
    ...
    .execute(pool)
    .await
    .context("inserting mission_completions")?;
    Ok(MissionCompletion {
        ...
        account_id: scope.account_id,
        completed_at,
        rewards_collected: Vec::new(),
    })
}

/// Remove a mission completion (cascades its collected-reward rows).
pub async fn remove_mission_completion(
    pool: &DbPool,
    scope: Scope,
    mission_id: &str,
) -> Result<bool> {
    let result = sqlx::query(
        "DELETE FROM mission_completions \
         WHERE mission_id = ? AND platform_id = ? AND account_id = ?",
    )
    .bind(mission_id)
    .bind(scope.platform.as_str())
    .bind(scope.account_id.to_string())
    .execute(pool)
    .await
    .context("deleting mission_completions")?;
    Ok(result.rows_affected() > 0)
}

pub async fn get_mission_completion(
    pool: &DbPool,
    scope: Scope,
    mission_id: &str,
) -> Result<Option<MissionCompletion>> {
    let row: Option<MissionRow> = sqlx::query_as(
        "SELECT id, mission_id, platform_id, account_id, completed_at FROM mission_completions \
         WHERE mission_id = ? AND platform_id = ? AND account_id = ?",
    )
    .bind(mission_id)
    .bind(scope.platform.as_str())
    ...
    .fetch_optional(pool)
    .await
    .context("selecting mission_completions")?;
    let Some(row) = row else { return Ok(None) };
    let mut mc = row_to_mission(row)?;
    mc.rewards_collected = load_rewards(pool, &mc.id).await?;
    Ok(Some(mc))
}

pub async fn list_mission_completions(
    pool: &DbPool,
    scope: Scope,
) -> Result<Vec<MissionCompletion>> {
    let rows: Vec<MissionRow> = sqlx::query_as(
        "SELECT id, mission_id, platform_id, account_id, completed_at FROM mission_completions \
         WHERE platform_id = ? AND account_id = ? ORDER BY completed_at",
    )
    .bind(scope.platform.as_str())
    .bind(scope.account_id.to_string())
    .fetch_all(pool)
    .await
    .context("listing mission_completions")?;
...
/// Record a collected non-repeatable BP reward under a completion (idempotent).
pub async fn add_reward_collected(
    pool: &DbPool,
    completion_id: RecordId,
    blueprint_guid: &str,
) -> Result<()> {
    sqlx::query(
        "INSERT OR IGNORE INTO mission_rewards_collected (mission_completion_id, blueprint_guid) \
         VALUES (?, ?)",
    )
    ...
    .execute(pool)
    .await
    .context("inserting mission_rewards_collected")?;
    Ok(())
}

pub async fn remove_reward_collected(
    pool: &DbPool,
    completion_id: RecordId,
    blueprint_guid: &str,
) -> Result<bool> {
    let result = sqlx::query(
        "DELETE FROM mission_rewards_collected \
         WHERE mission_completion_id = ? AND blueprint_guid = ?",
    )
    .bind(completion_id.to_string())
    .bind(blueprint_guid)
    .execute(pool)
    .await
    .context("deleting mission_rewards_collected")?;
    Ok(result.rows_affected() > 0)
}

async fn load_rewards(pool: &DbPool, completion_id: &RecordId) -> Result<Vec<String>> {
    let rows: Vec<(String,)> = sqlx::query_as(
        "SELECT blueprint_guid FROM mission_rewards_collected WHERE mission_completion_id = ?",
    )
    .bind(completion_id.to_string())
    .fetch_all(pool)
    .await
    .context("loading mission_rewards_collected")?;
    Ok(rows.into_iter().map(|(g,)| g).collect())
}

type MissionRow = (String, String, String, String, String);

fn row_to_mission(row: MissionRow) -> Result<MissionCompletion> {
    let (id, mission_id, platform_id, account_id, completed_at) = row;
    Ok(MissionCompletion {
        id: RecordId(id.parse().context("parsing record id")?),
        mission_id,
        platform: Platform::from_str(&platform_id)
...
mod tests {
    ...
    #[tokio::test]
    async fn mission_completion_roundtrip() {
        let pool = open_in_memory().await.unwrap();
        let a = account(&pool, "VeeLume").await;
        let s = scope(&a, Platform::Prod);

        // Mark completed (idempotent), collect two rewards.
        let mc = add_mission_completion(&pool, s, "mission-1").await.unwrap();
        assert!(mc.rewards_collected.is_empty());
        let again = add_mission_completion(&pool, s, "mission-1").await.unwrap();
        assert_eq!(again.id, mc.id);

        add_reward_collected(&pool, mc.id, "bp-a").await.unwrap();
        add_reward_collected(&pool, mc.id, "bp-b").await.unwrap();
        add_reward_collected(&pool, mc.id, "bp-a").await.unwrap(); // idempotent

        let loaded = get_mission_completion(&pool, s, "mission-1")
            .await
            .unwrap()
            .unwrap();
        assert_eq!(loaded.rewards_collected.len(), 2);
        assert_eq!(list_mission_completions(&pool, s).await.unwrap().len(), 1);

        assert!(remove_reward_collected(&pool, mc.id, "bp-a").await.unwrap());
        assert_eq!(
            get_mission_completion(&pool, s, "mission-1")
                .await
                .unwrap()
                .unwrap()
                .rewards_collected,
        ...
        // Un-completing cascades the remaining collected rewards.
        assert!(remove_mission_completion(&pool, s, "mission-1").await.unwrap());
        assert!(
            get_mission_completion(&pool, s, "mission-1")
                .await
                .unwrap()
        ...
        let orphans: Vec<(String,)> =
            sqlx::query_as("SELECT blueprint_guid FROM mission_rewards_collected")
                .fetch_all(&pool)
                .await
                .unwrap();
        assert!(orphans.is_empty(), "rewards should cascade-delete");
    }
}
