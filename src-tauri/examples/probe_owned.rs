...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cooked = hearth_lib::sc_loader::build_data(discovery.install)
        .await
        .expect("build data");
    let catalog = &cooked.blueprints;
    println!("catalog blueprints: {}", catalog.len());

    let bp_guids: HashSet<String> = catalog
        .iter()
        .map(|b| norm(&b.blueprint_record_guid))
        .collect();
    let entity_guids: HashSet<String> = catalog
        .iter()
        .filter_map(|b| b.crafted_entity_guid.as_deref().map(norm))
        .collect();
    let name_by_bp_guid: HashMap<String, String> = catalog
        .iter()
        .map(|b| {
            (
                norm(&b.blueprint_record_guid),
                b.display_name.clone().unwrap_or_default(),
            )
        })
        .collect();
    ...
    for o in &owned {
        if bp_guids.contains(&norm(&o.blueprint_id)) {
            bpid_bp += 1;
        }
        if bp_guids.contains(&norm(&o.item_class_id)) {
            iclass_bp += 1;
        }
        if entity_guids.contains(&norm(&o.item_class_id)) {
            iclass_entity += 1;
        }
        if bp_guids.contains(&norm(&o.category_id)) {
            cat_bp += 1;
        }
    }
    ...
    for o in owned.iter().take(15) {
        let name = name_by_bp_guid
            .get(&norm(&o.blueprint_id))
            .map(String::as_str)
